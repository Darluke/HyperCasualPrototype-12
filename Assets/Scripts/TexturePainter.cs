...
public class TexturePainter : MonoBehaviour {
	...
	public SpriteRenderer stickerRendere;
	...
	public string currentStickerName;
	
	void Update () {
		//brushColor = ColorSelector.GetColor ();	//Updates our painted color with the selected color
		//if (InputController.Instance.touchInput) {
		//	DoAction();
			
		//}
		UpdateBrushCursor ();
		stickerRendere.sprite = cursorDecal;
	}
	...
	void UpdateBrushCursor(){
		...
		if (HitTestUVPosition (ref uvWorldPosition) && !saving) {
			...
			brushCursor.transform.rotation = Quaternion.Euler(new Vector3(0, 0, 180));  // to fix the cursor rotation when we use x and y flip for the stickers
		} else {
			brushCursor.SetActive(false);
		}		
	}
